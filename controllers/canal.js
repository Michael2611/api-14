    ...
    try {
      ...
      if (canales_general.length >= 1) {
        ...
        for (var i of canales_general) {
          i.canales_populares = await usuario_amigo.countDocuments({
            usuario_amigo: i._id,
            tipo: "Canal",
          });
        }

        canales_general.sort(
          (a, b) => b.canales_populares - a.canales_populares
        );
        ...
        for (var item of amigos) {
          let canalesAmigo = await Canal.find({
            usuario_canal: item.usuario_amigo,
          });
        ...
        todosLosCanales = Array.from(
          new Set(todosLosCanales.map((canal) => canal._id))
        ...
        const canalesConSeguidores = await Promise.all(
          todosLosCanales.map(async (canal) => {
            ...
            const seguidoresCount = await usuario_amigo.countDocuments({
              usuario_amigo: canal._id,
            });
        ...
        res
          .status(200)
          .send({
            data: canalesConSeguidores,
            verificados: canales_verificados,
            populares: populares,
          });
      }
    } catch (error) {
...
const get_canal = async function (req, res) {
  if (req.user) {
    ...
    let seguidores = await usuario_amigo
      .find({ usuario_amigo: id })
      .populate("usuario_origen");
    let sigue_canal = await usuario_amigo.find({
    ...
    if (canal) {
      res
        .status(200)
        .send({ data: canal, sigue: sigue_canal, seguidores: seguidores });
    }
  } else {
    res.status(403).send({ message: "No Access" });
  }
...
const seguir_canal = async function (req, res) {
  if (req.user) {
    ...
    let verificar_seguimiento = await usuario_amigo.find({
      usuario_origen: req.user.sub,
      usuario_amigo: id,
    });
    ...
    if (verificar_seguimiento.length >= 1) {
      if (tipo == "Declinar") {
        await usuario_amigo.findOneAndDelete({
          usuario_origen: req.user.sub,
          usuario_amigo: id,
        }); //eliminar invitación
...
const delete_canal = async function (req, res) {
  if (req.user) {
    ...
    if (deletedCanal) {
      ...
      res
        .status(200)
        .send({ message: "Post deleted successfully", data: deletedCanal });
    } else {
...
const get_chat_canal = async function (req, res) {
  if (req.user) {
    ...
    if (chat_canal) {
      res
        .status(200)
        .send({ data: chat_canal, sigue: sigue_canal, canal: canal });
    }
  } else {
    res.status(403).send({ message: "No Access" });
  }
    ...
    if (messages_canal.length >= 1) {
      for (var item of messages_canal) {
        const imagePath = path.join(
          __dirname,
          "../uploads/canales/chat/" + item.media
        );
      ...
      res
        .status(200)
        .send({ message: "Post deleted successfully", data: d_messages_canal });
    } else {
      res.status(404).send({ message: "Post not found" });
    }
  } else {
...
const delete_message = async function (req, res) {
  if (req.user) {
    ...
    if (deletedMessage) {
      res
        .status(200)
        .send({ message: "Post deleted successfully", data: deletedMessage });
    } else {
      res.status(404).send({ message: "Post not found" });
    }
  } else {
...
const update_canal = async function (req, res) {
  if (req.user) {
    ...
    var canal = await Canal.findByIdAndUpdate(
      { _id: id },
      {
